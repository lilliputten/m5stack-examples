...
// extern unsigned char arial20[]; // Attempt failed

// Note the the tilda symbol ~ does not exist in some fonts at the moment
#define TEXT "Фыва abc 123 |" // Text that will be printed on screen in any font
...
void loop() {
  ...
  /* // Arial -- Attempt failed
  M5.Lcd.fillScreen(TFT_GREEN);            // Clear screen
  M5.Lcd.setFreeFont(FF18);                 // Select the font
  M5.Lcd.drawString("Arial 20", 160, 60, GFXFF); // Print the string name of the font
  M5.Lcd.setFreeFont(arial20);                 // Select the font
